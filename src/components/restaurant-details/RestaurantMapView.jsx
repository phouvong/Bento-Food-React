...
import { Accordion, AccordionDetails, AccordionSummary, Button, IconButton, styled, Typography } from "@mui/material";
...
const RestaurantMapView = ({ details, restaurantCoverUrl }) => {
    ...
    // };
    const openGoogleMaps = () => {
        if (details?.latitude && details?.longitude && currentLatLng?.lat,currentLatLng?.lng) {
    ...
    return (
        ...
        <Stack p="1rem" spacing={1}  minHeight={isSmall ? "70vh":"75vh"}>
            <StyledSimpleBar>
                <CustomImageContainer src={details.cover_photo_full_url}
                                      height="206px"
                                      objectFit="cover"
                                      width="100%" />
                ...
                <Stack direction="row" spacing={1} paddingBottom="16px" alignItems="center" justifyContent="space-between">
                    <Stack direction="row">
                ...
                {userLocation &&   <ResMapSection coords={coords} setUserLocation={setUserLocation}  userLocation={userLocation} details={details} />}
