        ...
        const filterData = combined?.map((item) => {
            return {
                ...
                products: item?.products?.filter((foods) => {
                    const isDiscountMatch = filterKey?.discount ? foods?.discount > 0 : true;
                    const isNonVegMatch = filterKey?.nonVeg ? foods?.veg === 0 : true;
                    const isVegMatch = filterKey?.veg ? foods?.veg === 1 : true;
                    const isAvailableMatch = filterKey?.currentlyAvailable
                        ? isAvailable(foods?.available_time_starts, foods?.available_time_ends)
                        : true;

                    return isDiscountMatch && isNonVegMatch && isVegMatch && isAvailableMatch;
                })
            };
        });
                                ...
                                return (
                                    <Box sx={{ position: "relative" }}>
                                        ...
                                        <CategoriesWiseFood
                                            ...
                                            hasFreeDelivery={restaurantData?.free_delivery}
                                        />
                                    </Box>
                                );
                            })}
