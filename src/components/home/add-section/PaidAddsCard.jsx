...
const PaidAddsCard = ({
    ...
    const slideHandler = () => {
        ...
        if (itemLength > 3 || isSmall) {
            if (!ended && item?.id === activeSlideData?.id && activeSlideData?.add_type === "video_promotion") {
                setPlaying(true);
            }
            return;
        }
        ...
        if (index === 2 && item.add_type === "video_promotion") {
            if (ended || (data[1]?.add_type !== "video_promotion" && data[0]?.add_type !== "video_promotion")) {
                setPlaying(true);
            }
        }
    };
        ...
        if (ended && data?.length > 0) {
            ...
            if (nextSlide < itemLength) {
                ...
                if (nextSlideChildren && nextSlideChildren[nextSlide]) {
                    const nextItem = nextSlideChildren[nextSlide]?.props?.children?.props?.item;
    ...
    const {
        ...
        isLoading,
        error
    } = useMutation("add-favourite", () => RestaurantsApi.addFavorite(item?.restaurant?.id), {
        onSuccess: (response) => {
    ...
    return (
        <>
            <Box sx={{ maxWidth: "450px", cursor: "pointer" }} onClick={handleClick}>
                            ...
                            {(item?.is_rating_active === 1 || item.is_review_active === 1) && (
                                ...
                                >
                                    {item.is_review_active === 1 && <>
                                        <StarIcon
                                            sx={{ fontSize: "18px", color: theme => theme.palette.neutral[100] }} />
                                        <Typography color={theme.palette.neutral[100]} fontSize="14px"
                                                    fontWeight="600">{item?.average_rating.toFixed(1)}</Typography>
                                    </>}
                                    {item.is_review_active === 1 && <Typography color={theme.palette.neutral[100]}
                                                                                fontSize="14px">({item?.reviews_comments_count}+)</Typography>}

                                </Stack>
                            )}

                            <CustomImageContainer
                                boxShadow={theme.palette.mode === 'dark'? "0px 15px 30px rgba(0, 0, 0, 0.8)" : "0px 15px 30px rgba(150, 150, 154, 0.40)"}
                    ...
                    {item?.add_type === "restaurant_promotion" ? (
                        <Stack flexDirection="row" gap="1rem" width="100%">
                            ...
                            <Stack width={0} flexGrow={1}>
                                <Stack flexDirection="row" gap=".6rem" width="100%" justifyContent="space-between"
                                       alignItems="center">
                                    <Typography
                                        sx={{
                                            overflow: "hidden",
                                    ...
                                    {!isInList(item?.restaurant?.id) ?
                                        <FavoriteBorderOutlinedIcon onClick={(e) => addToFavorite(e)}
                                                                    sx={{ cursor: "pointer", flexShrink: 0 }}
                                                                    color="primary" /> :
                                        <FavoriteIcon
                                            onClick={(e) => deleteWishlistRes(e)}
                                            color="primary"
                                            sx={{
                                                fontSize: {
                                                    xs: "16px",
                        ...
                        <>
                            ...
                            <Stack flexDirection="row" gap="20px" justifyContent="space-between">
                                ...
                                <IconButton
                                    onClick={handleClick}
                                    padding="10px"
                                    sx={{ borderRadius: "10px", border: `1.5px solid ${theme.palette.primary.main}` }}
                                >
            ...
            <CustomModal openModal={videoModal} closeButton setModalOpen={setVideoModal}>
